...
* Node.js (v18+)
* MongoDB (local or Atlas) and a Cloudinary account for image uploads
...
cd backend
npm install
cp .env.example .env   # fill in MONGODB_URI, ACCESS_TOKEN_SECRET, Cloudinary keys...
npm run dev
```

> `.env` files are git-ignored — never commit real secrets. Both `.env.example` templates document every required variable.
